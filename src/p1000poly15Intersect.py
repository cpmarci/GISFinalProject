...
testintersect = '''select a.pid,b.pyid,ST_Intersects(a.geom, b.geom), a.prefnum, a.ptime, b.pyref, b.pytime, ST_asText(a.geom)
from points1000 a, poly15 b WHERE a.ptime >= b.pytime and ST_Intersects(a.geom, b.geom) = True'''
...
for n in range(len(parseArray)):
    ...
    if holder[2] == True:
        ...
        tempOne = "INSERT INTO p1000poly15Intersect (geom) VALUES (ST_GeomFromText("+ "'" + holderTemp + "'" + ", 4269))"
        cur.execute(tempOne)
        conn.commit()
